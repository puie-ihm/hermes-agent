...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            async def handle_message_event(event, say, ack, body):
                await ack()
            ...
            async def handle_app_mention(event, say, ack, body):
                await ack()
                await self._handle_slack_message(event, body)

            @self._app.event("app_home_opened")
            async def handle_app_home_opened(event, say, ack, body):
                await ack()
                await self._handle_app_home_opened(event, body)

            @self._app.event("app_context_changed")
            async def handle_app_context_changed(event, say, ack, body):
                await ack()
            ...
            async def handle_file_shared(event, say, ack, body):
                await ack()
            ...
            async def handle_assistant_thread_started(event, say, ack, body):
                await ack()
                await self._handle_assistant_thread_lifecycle_event(event, body)

            @self._app.event("assistant_thread_context_changed")
            async def handle_assistant_thread_context_changed(event, say, ack, body):
                await ack()
                await self._handle_assistant_thread_lifecycle_event(event, body)
    ...
    async def _handle_slack_message(
        ...
        if not is_one_to_one_dm and bot_uid:
            ...
            if channel_id in self._slack_free_response_channels():
                ...
                # thread_followup_mode=agent: instead of dropping non-mention
                # follow-ups, let them reach the agent IF this is a thread the
                # bot participates in, so the agent can decide reply/react/silent.
                if self._thread_followup_mode() == "agent" and self._in_bot_thread(
